...
/***************************************************************
* 函数名称: PROJECT_IO_Init
* 说    明: PROJECT_GPIO初始化
* 参    数: 无
* 返 回 值: 无
***************************************************************/
static void PROJECT_IO_Init(void)
{
    GpioInit();
...
void PROJECT_Read_Data(void)
{
	...
	PROJECT_Data.Lux = (float)(((recv_data[0]<<8) + recv_data[1])/1.2);   
    ...
    if( !tmp ) /* value is ture */
    {
        ...
        PROJECT_Data.Temperature = SHT3x_CalcTemperatureC( dat );    
    }
    ...
    {
        dat = ((uint16_t)data[0] << 8) | data[1];
        PROJECT_Data.Humidity = SHT3x_CalcRH( dat );    
    }
